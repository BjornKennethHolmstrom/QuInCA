...
class DimensionalityReduction:
    def __init__(self, input_size, output_size):
        print(f"Initializing DimensionalityReduction with input_size={input_size}, output_size={output_size}")
        ...
        print(f"Projection matrix dtype: {self.projection_matrix.dtype}")
        print("DimensionalityReduction initialized successfully")
...
class ConvolutionalLayer:
    ...
    def forward(self, inputs):
        print(f"ConvolutionalLayer input shape: {inputs.shape}")
        ...
        print(f"ConvolutionalLayer output shape: {output.shape}")
        return output
...
class AttentionMechanism:
    ...
    def forward(self, inputs):
        print(f"AttentionMechanism input shape: {inputs.shape}")
        # Ensure inputs is 2D
        if len(inputs.shape) == 1:
            inputs = inputs.reshape(1, -1)
        ...
        print(f"AttentionMechanism output shape: {output.shape}")
        return output

class EfficientQuantumLayer:
    def __init__(self, input_size, output_size, sparsity=0.1):
        print(f"Initializing EfficientQuantumLayer with input_size={input_size}, output_size={output_size}")
        self.input_size = input_size
        self.output_size = output_size
        ...
        print(f"EfficientQuantumLayer connectivity_mask shape: {self.connectivity_mask.shape}")

    def forward(self, inputs):
        print(f"EfficientQuantumLayer input shape: {inputs.shape}")
        if len(inputs.shape) == 1:
            inputs = inputs.reshape(1, -1)
        ...
        print(f"EfficientQuantumLayer output shape: {outputs.shape}")
        return outputs
...
class EfficientQuantumNeuralNetwork:
    def __init__(self, input_size, conv_params, dim_reduction_size, attention_size, layer_sizes, sparsity=0.1):
        print(f"Initializing EfficientQuantumNeuralNetwork with parameters:")
        ...
        try:
            print("Creating ConvolutionalLayer")
            self.conv_layer = ConvolutionalLayer(input_size, **conv_params)
            print("ConvolutionalLayer created")
            ...
            print("Creating DimensionalityReduction")
            self.dim_reduction = DimensionalityReduction(conv_output_size, dim_reduction_size)
            print("DimensionalityReduction created")
            
            print("Creating AttentionMechanism")
            ...
            for i, size in enumerate(layer_sizes):
                print(f"Creating EfficientQuantumLayer {i+1}")
                self.layers.append(EfficientQuantumLayer(current_size, size, sparsity))
                print(f"EfficientQuantumLayer {i+1} created with input_size={current_size}, output_size={size}")
                current_size = size
            
            print("EfficientQuantumNeuralNetwork initialization complete")
        except Exception as e:
            print(f"Error in EfficientQuantumNeuralNetwork initialization: {e}")
            import traceback
            traceback.print_exc()
            raise

    def forward(self, inputs):
        print(f"EfficientQuantumNeuralNetwork forward pass, input shape: {inputs.shape}")
        x = self.conv_layer.forward(inputs)
        print(f"After conv layer, shape: {x.shape}")
        x = x.reshape(x.shape[0], -1)
        print(f"After reshaping, shape: {x.shape}")
        x = self.dim_reduction.forward(x)
        print(f"After dim reduction, shape: {x.shape}")
        x = self.attention.forward(x)
        print(f"After attention, shape: {x.shape}")
        for i, layer in enumerate(self.layers):
            print(f"Before quantum layer {i+1}, input shape: {x.shape}")
            x = layer.forward(x)
            print(f"After quantum layer {i+1}, output shape: {x.shape}")
        return x
